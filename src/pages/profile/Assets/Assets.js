...
// mock
import ASSETSLIST from '../../../_mock/assets';
...
export default function Assets() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = ASSETSLIST .map((n) => n.name);
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,index) => {
                    const { id, item,specification } = row;
                    const selectedUser = selected.indexOf(item) !== -1;
                    const SNo=index+1;

                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, item)} />
                        </TableCell>
